...
<head>
  ...
  <title>Math modeltest Main page</title>
  <meta charset="utf-8">
...
function submit_answer(){
  ...
  function next_question(){
	  ...
	  $(".results").html("");
	var bound = 20;
	$('#practice_numbera').text(1+Math.floor(Math.random() * bound));
	$('#practice_numberb').text(1+Math.floor(Math.random() *bound));
	$('#practice_numberc').text(1+Math.floor(Math.random() *bound));
	$('#practice_numberd').text(1+Math.floor(Math.random() * bound));	
  ...
  function show_answer(){
	  ...
	  var a = Number($('#practice_numbera').text());
	  var b = Number($('#practice_numberb').text());
	  var c = Number($('#practice_numberc').text());
	  var d = Number($('#practice_numberd').text());
	  var results = valuesFor4numbers(a,b,c,d);
	  
	  var htmlString = "";
	  for(var i=0; i < results.length; i++){
			 var val = eval(results[i]);
			 if(is24(val)){

				 htmlString = htmlString + strip(results[i])+"=24"+"<br>";
				 break;
			 }
		 }
		 if(htmlString=="") htmlString="No\\  solutions."
		htmlString = htmlString.replace("*", "\\times ");
			htmlString = htmlString.replace("*", "\\times ");
	      $(".results").html(htmlString); 
		  MQ.StaticMath(document.getElementById('latex24'));
		  clearInterval(x);
  }
...
<div class="container-fluid text-center">    
  <div class="row content">
	...
	<li><a ui-sref="game" ui-sref-active="active">Game</a> </li>
	
	<li><a ui-sref="practice2" ui-sref-active="active">Practice2</a> </li>
